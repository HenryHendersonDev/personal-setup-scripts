...
apt install -y zsh curl git nano dbus dbus-x11 gnome-shell-extensions gnome-tweaks wget preload gnome-core xorg network-manager-gnome sudo unzip || log_error "Package installation failed."

# Uninstall Some packages
log_info "Removing unwanted packages..."
apt remove --purge gnome-calculator gnome-characters gnome-contacts gnome-software totem gnome-system-monitor firefox-esr -y

log_info "Installing some replacements..."
apt install -y nautilus gnome-calculator gnome-system-monitor || log_warning "Failed to install some packages."
